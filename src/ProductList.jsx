 ...
 const showAddedToCartItems = () => {
    let items = 0;
     cartItems.forEach((item) => {
      items += item.quantity; // Sum up the quantities of all items in the cart
    });
    return items > 0 ? items : ""; // Return the total quantity or an empty string if no items
  }

  const cartTotalItems = showAddedToCartItems(); // Call the function to get the total items in the cart
            ...
            <div key={index}> {/* Unique key for each category div */}
                ...
                {category.plants.map((plant, plantIndex) => ( // Loop through each plant in the current category
                    <div className="product-card" key={plantIndex}> {/* Unique key for each plant card */}
                        ...
                        {!addedToCart[plant.name] ? ( // Check if the plant is already added to the cart}
